...
| **`worker/`** | The extraction worker. `listener.py` blocks on `LISTEN` until a job arrives, `jobqueue.py` claims/retries/reclaims rows, `jobs.py` runs extraction → grounding → persist. Bundled into the API image; it has no container of its own. |
...
**Why Postgres as the queue:** the job's state (`pending → processing → done/error`) already lived in the `extractions` table — a broker would only duplicate it. `NOTIFY` gives the worker an instant wake-up; `FOR UPDATE SKIP LOCKED` makes claiming a row atomic and safe even if several workers run.
...
2. The **worker thread** wakes from `LISTEN` and claims the oldest `pending` row (`status → processing`).
3. It sends the PDF to **Gemini** with the clinical schema in the prompt and validates the JSON against `PatientChart`. Individually invalid values are dropped and recorded in `flagged` so the rest of the chart still validates.
4. It **grounds** the result: for every `SourceRef`, the cited quote must appear on the cited page (per-page text via `pypdf`, segment-based fuzzy match).
5. It writes `status = done` with the immutable original chart, an editable working copy, the grounding report, and the flags — and **clears the PDF bytes**, which are only held while the job is in flight.
6. The **web UI** polls **`GET /charts/{job_id}`** until `done`, renders the chart with per-fact source chips, and saves edits via **`PUT /charts/{job_id}`**.
...
- A row stuck in `processing` for **20 min** (worker died mid-job) is requeued. That counts as a failed attempt, so a job that crashes the worker every time can't loop forever.
...
├── worker/         # Extraction worker: listener, job queue, extraction + grounding
...
The database is the source of truth: the row id *is* the job id.

---

## Getting started (local)

Requires Docker, Node.js, and a Google Gemini API key (needed for extraction; not for `/health` or unit tests).

```bash
cp .env.example .env          # dev defaults work; add GEMINI_API_KEY=... for extraction
...
Two services on Render, both free tier: the API as a **Docker web service** (build context is the repo root, so the image can see `shared/` and `worker/`) and the frontend as a **static site**. Render auto-deploys on every push to `main`; migrations are applied manually against Neon when a new one lands. Setup details are in `CLAUDE.md`.
