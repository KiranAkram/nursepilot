...
from jobqueue import MAX_ATTEMPTS, claim, fail, reclaim_stale
...
def test_claim_takes_oldest_pending_and_marks_processing(engine, conn):
    ...
    assert _row(engine, older).status == "processing"
...
def test_fail_retries_then_errors_and_drops_pdf(engine, conn):
    job_id = _insert(engine, status="processing", pdf=b"%PDF")
...
def test_reclaim_stale_requeues_only_old_processing_rows(engine, conn):
    stale = _insert(engine, status="processing", pdf=b"%PDF")
    fresh = _insert(engine, status="processing", pdf=b"%PDF")
    conn.execute(
